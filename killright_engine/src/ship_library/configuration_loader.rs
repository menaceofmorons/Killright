...
use crate::ship_library::ship_classification_models::ShipClassificationConfiguration;

pub fn load_default_configuration() -> Result<ShipClassificationConfiguration, String> {
    let candidates = configuration_candidates();
...
pub fn load_configuration(path: &Path) -> Result<ShipClassificationConfiguration, String> {
    ...
    let configuration = serde_json::from_str::<ShipClassificationConfiguration>(text)
