...
use crate::pintel_engine::threat_analysis::threat_configuration_models::ThreatConfiguration;

pub fn load_default_threat_configuration()
    -> Result<ThreatConfiguration, String>
{
...
pub fn load_threat_configuration(
    path: &Path)
    -> Result<ThreatConfiguration, String>
{
    ...
    validate_threat_configuration(&configuration)
        .map_err(|error|
            format!(
...
fn validate_threat_configuration(
    ...
    -> Result<(), String>
{
    ...
    if expected_minimum != 101 {
        return Err(
            "threat bands must cover score range 0-100".to_string());
    }

    Ok(())
}

fn validate_non_negative_scores(
    configuration: &ThreatConfiguration)
    -> Result<(), String>
{
    let weights = &configuration.component_weights;
